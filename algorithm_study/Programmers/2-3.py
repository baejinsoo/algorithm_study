...
def solution(program, flag_rules, commands):
    ...
    # 예시 flags = {'-s': 'STRING', '-num': 'NUMBER', '-e': 'NULL', '-n': 'NUMBER'}
    flags = dict()
    for flag in flag_rules:
        ...
        # alias를 딕셔너리에 추가할 경우
            ...
            alias_name, alias, flag_name = flag.split()
            # alisa가 먼저 선언된 경우
            if flag_name not in flags:
                continue
            flags.update({alias_name: flags[flag_name]})
    # alisa가 먼저 선언된 경우 alias를 딕셔너리에 추가하기위해 다시한번 실행
    for flag in flag_rules:
        if len(flag.split()) == 3:
            alias_name, alias, flag_name = flag.split()
            flags.update({alias_name: flags[flag_name]})
    ...
    for k in range(len(commands)):
        ...
        for co in com:
            if co in flags:
                if flags[co] in chk_alias:
                    answer[k] = False
                else:
                    chk_alias.append(flags[co])
